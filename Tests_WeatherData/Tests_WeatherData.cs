...
namespace Tests_WeatherData
{
    ...
    {
        private static string filename = @"Environmental_Data_Deep_Moor_2012.txt";
...
2012_01_01 00:21:00	33.80	30.60	27.30	76.60	280.80	17.00	14.00";
        ...
        public void Test_060_FitLineToFilteredSampleFile()
        {
            ...
            using (var text = new StreamReader(filename))
            {
                ...
                // Extract
                var wo = WeatherData.ReadRange(text, start, end);

                // Transform
                var data = wo.Select(s => new {
                    Hours = (s.TimeStamp - s.TimeStamp).TotalHours,
                ...
                {
                    arrX.Add(item.Hours);
                    arrY.Add(item.Barometric_Pressure);
                }
