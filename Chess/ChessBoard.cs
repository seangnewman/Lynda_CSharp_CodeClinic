...
namespace Chess
{
    public class ChessBoard
    {
        ...
        public bool IsSafe()
        {
            ...
            var countZeroes = Board.Count(n => n == 0);      // Number of unoccupied rows
            var countDistinct = Board.Distinct().Count();    // Number of occupied rows
            if (Board.Length != countDistinct + (countZeroes > 1 ? countZeroes - 1 : 0))
            {
                // Two or more queens on same row
                return false;
            }
            ...
            for (int i = 1; i <= Board.Count(); i++)
            {
                for (int j = i + 1; j <= Board.Count(); j++)
                {
                    if (Board[i - 1] != 0 && Board[j - 1] != 0)
                    {
                        ...
                        // determine slope, if the slope is 1...then can be attacked
                        if (dX == dY)
                        {
                            return false;
                        }
